...
def distance(a,b):
    # returns the l2 norm between two points
    d = 0
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    if not len(left_list) > 1:
        return None
    lc = compute_center(left_list)
    rc = compute_center(right_list)
    l_print(f'Left center: {lc}, Right center: {rc}')
    ...
    for i in range(len(left_list)):
        ...
        l_print(f'\trx: {rx}, ry: {ry}, lx: {lx}, ly: {ly}')
    ...
    l_print(f'cs: {cs}, ss: {ss}, rr: {rr}, ll: {ll}, len_cs_ss: {len_cs_ss}')
    c = cs / len_cs_ss
    s = ss / len_cs_ss
