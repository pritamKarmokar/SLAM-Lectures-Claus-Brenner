...
def kalman_filter_step(belief, control, measurement):
    ...
    # Prediction.
    prediction = Density(a * belief.mu + control.mu, a**2 * belief.sigma2 + control.sigma2) 

    # Correction.
    kalman_gain = (c * prediction.sigma2) / (c**2 * prediction.sigma2 + measurement.sigma2)

    mu = prediction.mu + kalman_gain * (measurement.mu - c * prediction.mu)
    sigma2 = (1 - kalman_gain * c) * prediction.sigma2
